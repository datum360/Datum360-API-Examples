...
# Datum360 API
To get started with the API we recommend taking a look at the API docs on each system (CLS360, PIM360, DDM360). To access the API docs, log into a system and then navigate to /api-docs e.g
`https://example-system.cls360.io/api-docs`

You can also navigate to it by clicking on the top right of the screen in the PIM360/CLS360/DDM360 user interface and then selecting "API Documentation"
...
To authenticate with a system (PIM360, CLS360, DDM360) replace "example-system" in the authorization and token urls with the ACL url of the system you are trying to authenticate with. For example in NodeJS, using passport we use: 
...
You must have a valid account for the system you are trying to authenticate with, enter the username and password for this account against clientID and clientSecret

You must include the services (PIM360, CLS360, DDM360) you wish to authenticate against in the scope of the passport request, this can be an array to include multiple services.
...
where `username` and `password` should be replaced with valid credentials for the PIM360/CLS360/DDM360 system you are trying to authenticate against. We recommened having a "service account" for API usage rather than a users personal account.

`example-system-pim360` should be replaced with the system you're tryign to authenticate with, this can be found in the URL e.g `https://example-system.pim360.io/`. For example:
...
Submitting the export activity request will return details about the activity that has been submitted, however at this point the activity will not have completed. So from the activity we need to get the activity handle and submit it to the get activity endpoint, we repeat this until the activity has completed, either successfully or unsuccessfully. To do this we have some helper functions such as `checkActivtyStatus` in the JavaScript examples
...
The messaging code example shows how to check for cls360 messages and then read any available messages. You will need to include your own connection string and subscription name

When a message is received it will have the message properties available within message.applicationProperties. So in the example you could get the handle of a newly created class by doing:
